...
//privilege:string,grantable:string},..]
    async getAllPrivileges(connectionAdmin) {
        ...
        const [sysPrivs, tabPrivs, colPrivs] = await Promise.all([getAllSysPrivs(connectionAdmin),
        getAllTabPrivs(connectionAdmin), getAllColPrivs(connectionAdmin)]);

        for (const row of sysPrivs.rows) {
            result.push({
                grantee: row.GRANTEE, owner: null, table: null, column: null, grantor: null,
        ...
        for (const row of tabPrivs.rows) {
            result.push({
                grantee: row.GRANTEE, owner: row.OWNER, table: row.TABLE_NAME, column: row.COLUMN_NAME,
...
//privilege:string,grantable:string},..]
    async getAllPrivsByUsername(connectionAdmin, username) {
        ...
        getTabPrivsByUsername(connectionAdmin, username), getSysPrivsByUsername(connectionAdmin, username)]);
        ...
        for (const row of colPrivs.rows) {
            result.push({
                grantee: row.GRANTEE, owner: row.OWNER, table: row.TABLE_NAME,
                column: row.COLUMN_NAME, privilege: row.PRIVILEGE, grantable: row.GRANTABLE
            })
        }
        ...
        for (const row of tabPrivs.rows) {
            result.push({
                grantee: row.GRANTEE, owner: row.OWNER, table: row.TABLE_NAME,
                column: null, privilege: row.PRIVILEGE, grantable: row.GRANTABLE
            })
        }

        for (const row of sysPrivs.rows) {
            result.push({
                grantee: row.GRANTEE, owner: null, table: null,
                column: null, privilege: row.PRIVILEGE, grantable: row.ADMIN_OPTION
            })
        }
        return result;
    },
//TRUNG ROIIIIIIIIIIIIIIIIIIIIIIII, BEN TREN ROLE CO NAY ROI
    async getAllPrivsByRoleName(connectionAdmin, roleName) {
        ...
        const [sysPrivs, tabPrivs, colPrivs] =
            await Promise.all([getSysPrivsGrantedByRoleByRolename(connectionAdmin, username),
        ...
        for (const row of sysPrivs.rows) {
            result.push({
                grantee: row.GRANTEE, owner: nullR, table: null,
                column: null, privilege: row.PRIVILEGE, grantable: row.ADMIN_OPTION
            })
        }
        return result;
