...
const VALID = 1;
const PROFILE_VALUE_INVALID =
  "Profile value must be default or unlimited or a number";
const PROFILE_NAME_INVALID = "Profile name must be string";

function checkProfileInfo(profileData) {
  const profileDefaultValue = "DEFAULT";
  const profileUnlimitValue = "UNLIMITED";

  if (isNaN(profileData.profile) === false) return PROFILE_NAME_INVALID;
  const { profile, ...other } = profileData;

  for (const field in other) {
    if (
      isNaN(other[field]) &&
      other[field].trim().toUpperCase() !== profileDefaultValue &&
      other[field].trim().toUpperCase() !== profileUnlimitValue
    )
