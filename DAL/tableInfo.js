...
const getAllSysPrivs = (connectionAdmin) => {
    const getSysPrivsQuery = "select GRANTEE,PRIVILEGE,ADMIN_OPTION from DBA_SYS_PRIVS WHERE INHERITED='NO'";

    return initializeQuery(connectionAdmin, getSysPrivsQuery);
}

const getAllTabPrivs = (connectionAdmin) => {
    const getTabPrivsQuery = "select GRANTEE,OWNER,TABLE_NAME,GRANTOR,PRIVILEGE,GRANTABLE,TYPE " +
...
const getAllColPrivs = (connectionAdmin) => {
    const getColPrivsQuery = "select GRANTEE,OWNER,TABLE_NAME,COLUMN_NAME,GRANTOR,PRIVILEGE,GRANTABLE " +
...
const getAllRoles = (connectionAdmin) => {
    const getRolesQuery = "select ROLE_ID,ROLE from dba_roles where inherited = 'NO'";

    return initializeQuery(connectionAdmin, getRolesQuery);
}

const getSysPrivsByRole = (connectionAdmin, roleName) => {
    const getSysPrivsByRoleQuery = "select grantee,privilege,admin_option from dba_sys_privs where grantee = '"+roleName+"'";

    return initializeQuery(connectionAdmin, getSysPrivsByRoleQuery);
}

const getTabPrivsByRole = (connectionAdmin, roleName) => {
    const getTabPrivsByRoleQuery = "select grantee,owner,table_name,grantor,privilege,grantable from dba_tab_privs "+
...
const getColPrivsByRole = (connectionAdmin, roleName) => {
    const getColPrivsByRoleQuery = "select grantee,owner,table_name,column_name,grantor,privilege,grantable "+
" from dba_col_privs where grantee='"+roleName+"'";

    return initializeQuery(connectionAdmin, getColPrivsByRoleQuery);
}

const getUsersByRole = (connectionAdmin, roleName) => {
    const getUsersByRoleQuery = "select grantee,granted_role,admin_option from dba_role_privs WHERE GRANTED_ROLE='" + roleName + "'";
...
const getProfiles = (connectionAdmin) => {
    const getProfilesQuery = "SELECT NAME FROM PROFNAME$";

    return initializeQuery(connectionAdmin, getProfilesQuery);
}

const getResourcesByProfile = (connectionAdmin, profileName) => {
    const getUsersByRoleQuery = "select PROFILE,RESOURCE_NAME,LIMIT from DBA_profileS where profile = '" + profileName +
...
const getUsersByProfile = (connectionAdmin, profileName) => {
    const getUsersByProfileQuery = "select username from DBA_USERS where profile = '" + profileName + "'";
...
const getAllUserInfo = (connectionAdmin) => {
    const getAllUserInfoQuery = "SELECT DBA_USERS.username,DBA_USERS.account_status,DBA_USERS.lock_date, " +
...
const getUserInfoByUsername = (connectionAdmin, username) => {
    const getAllUserInfoBQuery = "SELECT DBA_USERS.username,DBA_USERS.account_status,DBA_USERS.lock_date, " +
        "DBA_USERS.created, DBA_USERS.default_tablespace,dba_ts_quotas.max_bytes as quotas, " +
        " DBA_USERS.temporary_tablespace,DBA_USERS.profile FROM DBA_USERS  inner join dba_ts_quotas " +
        " on DBA_USERS.username = dba_ts_quotas.username " +
...
const getUserInfoByUser = (connection) => {
    const getUserInfoByUserNameQuery = "SELECT USER_USERS.username,USER_USERS.account_status,USER_USERS.lock_date, " +
...
const getRolesByUser = (connection) => {
    const getRolesByUserQuery = "SELECT granted_role,admin_option FROM USER_ROLE_PRIVS";

    return initializeQuery(connection, getRolesByUserQuery);
}

const getRolesByUsername = (connectionAdmin, username) => {
    const getRolesByUserNameQuery = "SELECT grantee,granted_role,admin_option FROM dba_ROLE_PRIVS where grantee = '" + username + "'";

    return initializeQuery(connectionAdmin, getRolesByUserNameQuery);
}

const getColPrivsByUser = (connection) => {
    const getColPrivsByUserQuery = "SELECT owner,table_name,column_name,grantor,privilege,grantable " +
...
const getColPrivsByUsername = (connectionAdmin, username) => {
    const getColPrivsByUsernameQuery = "SELECT grantee,owner,table_name,column_name,grantor,privilege,grantable " +
        " FROM dba_col_PRIVS where grantee = '" + username + "'";

    return initializeQuery(connectionAdmin, getColPrivsByUsernameQuery);
}

const getTabPrivsByUser = (connection) => {
    const getTabPrivsByUserQuery = "SELECT owner,table_name,grantor,privilege,grantable FROM USEr_TAB_PRIVS " +
        " where grantee = USER";
...
const getTabPrivsByUsername = (connectionAdmin, username) => {
    const getTabPrivsByUsernameQuery = "SELECT grantee,owner,table_name,grantor,privilege,grantable FROM dba_TAB_PRIVS " +
...
const getSysPrivsByUser = (connection) => {
    const getSysPrivsByUserQuery = "SELECT privilege,admin_option FROM USEr_sys_PRIVS";

    return initializeQuery(connection, getSysPrivsByUserQuery);
}

const getSysPrivsByUsername = (connectionAdmin, username) => {
    const getSysPrivsByUsernameQuery = "SELECT grantee,privilege,admin_option FROM dba_sys_PRIVS " +
        " where grantee = '" + username + "'";
...
const getSysPrivsGrantedByRoleByUser = (connection) => {
    const getSysPrivsGrantedByRoleByUserQuery = "SELECT role,privilege,admin_option FROM ROLE_SYS_PRIVS";
...
const getSysPrivsGrantedByRoleByRolename = (connectionAdmin, rolename) => {
    const getSysPrivsGrantedByRoleByRolenameQuery = "select GRANTEE,PRIVILEGE,ADMIN_OPTION FROM DBA_SYS_PRIVS where " +
        " grantee = '" + rolename + "'";

    return initializeQuery(connectionAdmin, getSysPrivsGrantedByRoleByRolenameQuery);
}

const getTabPrivsGrantedByRoleByUser = (connection) => {
    const getTabPrivsGrantedByRoleByUserQuery = "SELECT role,owner,table_name,column_name,privilege,grantable " +
...
const getTabPrivsGrantedByRoleByRolename = (connectionAdmin, rolename) => {
    const getTabPrivsGrantedByRoleByRolenameQuery = "SELECT grantee,owner,table_name,grantor,privilege,grantable " +
        " FROM DBA_TAB_PRIVS where GRANTEE = '" + rolename + "'";

    return initializeQuery(connectionAdmin, getTabPrivsGrantedByRoleByRolenameQuery);
}

const getColPrivsGrantedByRoleByRolename = (connectionAdmin, rolename) => {
    const getColPrivsGrantedByRoleByRolenameQuery = "SELECT grantee,owner,table_name,column_name,grantor,privilege,grantable " +
        " FROM DBA_COL_PRIVS where GRANTEE = '" + rolename + "'";

    return initializeQuery(connectionAdmin, getColPrivsGrantedByRoleByRolenameQuery);
}
